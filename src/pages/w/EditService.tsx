...
export default function WorkerEditService() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const service = id ? store.getService(id) : undefined;
  ...
  const [form, setForm] = useState({
    ...
    withUrgency: service?.withUrgency ?? false,
  });

  if (!service) {
    return (
      <div>
        <PageHeader title="Servicio no encontrado" back />
        <Card>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error((err as Error).message || "Error al actualizar el servicio");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <PageHeader title="Editar servicio" subtitle="Actualizá los datos de tu servicio." back />
